...
public class JMSEventBusTransport implements IEventBusTransport,
	...
	try {
	    // Incoming absurdity to deal with (potential?) bug in ActiveMQ Java in which
	    // messages come twice to the listener
	    // TODO: Check if this is really the expected behaviour of ActiveMQ
	    logger.debug(arg0.getJMSMessageID() + ":" + arg0.getJMSTimestamp() + ":" + arg0.getJMSDestination());
	    newID = arg0.getJMSMessageID();
	    isDifferent = !(newID.equals(lastID));
	    lastID = newID;
	} catch (JMSException e1) {
	    logger.error("Error while retrieving message JMS properties: " + e1.getMessage());
	    // For reliability reasons, better to let pass this one, better to receive the message anyway...
	    isDifferent = true;
	}
