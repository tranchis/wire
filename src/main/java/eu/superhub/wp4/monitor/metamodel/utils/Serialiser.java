	...
	private void initialise(Class<?> factory, String extension) {
		...
		try {
			resourceSet.getPackageRegistry().put(
					(String) factory.getDeclaredField("eNS_PREFIX")
							.get(factory),
					factory.getDeclaredField("eINSTANCE").get(factory));
			resourceSet.getPackageRegistry().put(
					(String) factory.getDeclaredField("eNS_URI").get(factory),
					factory.getDeclaredField("eINSTANCE").get(factory));
			resourceSet.getPackageRegistry().put(NotationPackage.eNS_PREFIX,
		...
		if (bCrossRef) {
			...
			try {
				while (it.hasNext()) {
					...
					// if(!resource.getContents().contains(recurse))
					if ((!resource.getContents().contains(recurse))
							&& (!resource.equals(recurse.eResource()))) {
				...
				if (addCounter > 10) {
					System.err
							.println("tried adding to resource too many time.  give up");
					return;
				}
