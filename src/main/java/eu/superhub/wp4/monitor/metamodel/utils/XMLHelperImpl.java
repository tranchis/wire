...
/**
 * This class handles the package to use when there is no XML namespace in an
 * XML file.
 */
public class XMLHelperImpl implements XMLHelper {
    ...
    public static String saveString(Map<?, ?> options,
	    List<? extends EObject> contents, String encoding, XMLHelper helper)
	    throws Exception {
	if (helper == null) {
	    helper = new XMIHelperImpl();
	}
	if (!options.containsKey(XMLResource.OPTION_DECLARE_XML)) {
	    Map<Object, Object> modifiedOptions = new HashMap<Object, Object>(
		    options);
	...
	if (Boolean.TRUE.equals(options
		.get(XMLResource.OPTION_DEFER_IDREF_RESOLUTION))) {
	    ((XMLHelperImpl) helper).checkForDuplicates = true;
	}

	((XMLHelperImpl) helper).processDanglingHREF = (String) options
	...
	prefixesToURIs = new BasicEMap<String, String>() {
	    ...
	    @Override
	    protected void didClear(
		    BasicEList<Entry<String, String>>[] oldEntryData) {
		urisToPrefixes.clear();
	    }

	    @Override
	    protected void didModify(Entry<String, String> entry,
		    String oldValue) {
	...
	laxFeatureProcessing = Boolean.TRUE.equals(options
		.get(XMLResource.OPTION_LAX_FEATURE_PROCESSING));
	uriHandler = (XMLResource.URIHandler) options
		.get(XMLResource.OPTION_URI_HANDLER);
	if (uriHandler != null) {
	    uriHandler.setBaseURI(resourceURI);
	}
	@SuppressWarnings("unchecked")
	List<? extends EObject> roots = (List<? extends EObject>) options
	...
	if (roots != null) {
		...
		for (InternalEObject container = internalEObject
			...
			uriFragmentPath.add(index != 0 ? Integer
				.toString(index) : "");
			break;
		    }
		}
	...
	return noNamespacePackage != null ? noNamespacePackage
		: extendedMetaData != null ? extendedMetaData.getPackage(null)
			: null;
    }
	...
	if (extendedMetaData != null
		&& extendedMetaData.getPackage(null) != null) {
	...
	if (resource == null) {
	    ...
	    packageRegistry = resource.getResourceSet() == null ? EPackage.Registry.INSTANCE
    ...
    protected String getQName(NameInfo nameInfo, EPackage ePackage,
	    String name, boolean mustHavePrefix) {
	...
	if (nsPrefix == null || mustHavePrefix && nsPrefix.length() == 0) {
	    ...
	    if (!found) {
		// for any content prefix to URI mapping could be in namespace
		// context
		...
		if (nsURI != null) {
		    nsPrefix = xmlSchemaTypePackage == ePackage ? "xsd"
		...
		if (prefixesToURIs.containsKey(nsPrefix)) {
		    ...
		    if (currentValue == null ? nsURI != null : !currentValue
			    .equals(nsURI)) {
			int index = 1;
			while (prefixesToURIs.containsKey(nsPrefix + "_"
				+ index)) {
			    ++index;
			}
			nsPrefix += "_" + index;
	...
	String namespace = extendedMetaData == null ? ePackage.getNsURI()
		: extendedMetaData.getNamespace(ePackage);
	...
	EPackage ePackage = extendedMetaData == null ? EPackage.Registry.INSTANCE
		.getEPackage(uri) : extendedMetaData.getPackage(uri);
	if (ePackage == null) {
	    if (extendedMetaData != null) {
	...
	if (roots != null && containingResource == resource
	    ...
	    Iterator<EObject> it = containingResource.getContents().iterator();
	    int j = 0;
	    EObject eo = it.next();
	    while (it.hasNext() && eo != object) {
		eo = it.next();
		j++;
	    }
	    if (eo == object) {
	    ...
	    // System.outprintln("getURIFragment = " + result);
	    // System.outprintln(containingResource + ": " +
	    // containingResource.getContents());
		...
		for (int i = 0; i < fragmentPrefixes.length; ++i) {
			...
			result = "/" + (i == 0 ? "" : Integer.toString(i))
				+ result.substring(fragmentPrefix.length() - 1);
			break;
		    }
		}
	    }
	...
	if (!XMLResource.OPTION_PROCESS_DANGLING_HREF_DISCARD
		.equals(processDanglingHREF)) {
	    DanglingHREFException exception = new DanglingHREFException(
		    "The object '" + object
			    + "' is not contained in a resource.",
		    resource == null || resource.getURI() == null ? "unknown"
			    : resource.getURI().toString(), 0, 0);
	...
	if (feature == null) {
		...
		List<EStructuralFeature> structuralFeatures = eClass
			.getEAllStructuralFeatures();
		for (int i = 0, size = structuralFeatures.size(); i < size; ++i) {
		    EStructuralFeature eStructuralFeature = structuralFeatures
			    .get(i);
		    if (name.equals(extendedMetaData
			    .getName(eStructuralFeature))
			    && (namespaceURI == null ? extendedMetaData
				    .getNamespace(eStructuralFeature) == null
				    : namespaceURI.equals(extendedMetaData
					    .getNamespace(eStructuralFeature)))) {
	...
	if (extendedMetaData != null) {
	    // Once we see a lookup of an element in the null namespace, we
	    // should behave as if there has been an explicit xmlns=""
	    //
	    if (isElement && namespaceURI == null) {
		seenEmptyStringMapping = true;
	    }
	    EStructuralFeature eStructuralFeature = isElement ? extendedMetaData
		    .getElement(eClass, namespaceURI, name) : extendedMetaData
		    .getAttribute(eClass, namespaceURI == "" ? null
			    : namespaceURI, name);
		...
		// Only if the feature kind is unspecified should we return a
		// match.
		// Otherwise, we might return an attribute feature when an
		// element is required,
		// or vice versa. This also can be controlled by
		// XMLResource.OPTION_LAX_FEATURE_PROCESSING.
		//
		if (!laxFeatureProcessing
	...
	// Sort and eliminate duplicates caused by having both a regular package
	// and a demanded package for the same nsURI.
	//
	...
	if (extendedMetaData != null) {
		...
		if (targetFeature.getEType() == EcorePackage.Literals.EFEATURE_MAP_ENTRY) {
			...
			// Create an simple any type wrapper for the attribute
			// value and use that with the EReference.
			//
			SimpleAnyType simpleAnyType = (SimpleAnyType) EcoreUtil
				.create(anySimpleType);
	...
	case DATATYPE_IS_MANY: {
		...
		object.eSet(feature,
			createFromString(eFactory, eDataType, (String) value));
	    }
	    break;
	}
	case IS_MANY_ADD:
    ...
    public List<XMIException> setManyReference(ManyReference reference,
	    String location) {
	...
	if (kind == IS_MANY_ADD) {
		...
		if (value != null) {
			...
			xmiExceptions.add(new IllegalValueException(object,
				feature, value, e, location, reference
	...
	return uriHandler == null ? relative.resolve(base) : uriHandler
		.resolve(relative);
    }
	...
	return "xml".equals(prefix) ? "http://www.w3.org/XML/1998/namespace"
		: "xmlns".equals(prefix) ? ExtendedMetaData.XMLNS_URI
	...
	for (int i = 0, size = allPrefixToURI.size(); i < size;) {
		...
		seenEmptyStringMapping = true;
		if (originalURI != null) {
		    // since xmlns="" is default declaration, remove
		    // ""->empty_URI mapping
		...
		// record default ns declaration as duplicate if seen QName (#1)
		// or seen xmlns="" (#2)
    ...
    public void setPrefixToNamespaceMap(
	    EMap<String, String> prefixToNamespaceMap) {
	 ...
	 * @return true if the prefix existed in the current context and its uri
	 *         has been remapped; false if prefix does not exist in the
	 *         current context
	 */
	...
	public String getPrefix(String uri) {
		...
		if ((knownURI != null) ? knownURI.equals(uri) : uri == knownURI) {
		    ...
		    if ((knownURI != null) ? knownURI.equals(uri)
			    : uri == knownURI)
	...
	if (value instanceof QName) {
		...
		throw new IllegalArgumentException(
			"Curly brace notation is not a syntactically valid serialized representation for the QName '"
				+ qName.toString() + "'");
	    }
	    String prefix = qName.getPrefix();
	    String namespace = getURI(prefix);
	    qName = new org.eclipse.emf.ecore.xml.type.internal.QName(
		    namespace, qName.getLocalPart(), prefix);
	    if (qName.getPrefix().length() > 0 && namespace == null) {
		throw new IllegalArgumentException("The prefix '" + prefix
			+ "' is not declared for the QName '"
			+ qName.toString() + "'");
	    }
	    if (namespace == null) {
		seenEmptyStringMapping = true;
     ...
     * @param dataType
     * @param value
     *            a data value to be converted to string
     * @param list
     *            if the value is part of the list of values
     * @return if the value is not part of the list, return string corresponding
     *         to value, otherwise return null
     */
	...
	if (value instanceof QName) {
		...
		if (value instanceof org.eclipse.emf.ecore.xml.type.internal.QName) {
		    ...
		    throw new IllegalStateException(
			    "The null namespace cannot be bound to a non-null prefix '"
				    + qName + "'");
		}
		return localPart;
	    }
		...
		ePackage = extendedMetaData.demandPackage(namespace);
		if (prefix.length() != 0
			&& extendedMetaData.demandedPackages().size() > size) {
	    ...
	    return list ? null : prefix.length() == 0 ? localPart : prefix
		    + ':' + localPart;
	}
	...
	if (uri != null) {
		...
		int index = 1;
		String newPrefix;
		while (prefixesToURIs.containsKey(newPrefix = prefix + "_"
			+ index)) {
		    lowerBound = index;
		    index <<= 1;
		}
