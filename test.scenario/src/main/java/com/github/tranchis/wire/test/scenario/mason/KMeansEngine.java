...
public class KMeansEngine
{
	...
	Double2D[] clusterPoints;// = new Double2D[
								// CooperativeObservation.NUM_AGENTS ];
	...
	public final double distanceBetweenPointsSQR(double x1, double y1,
			double x2, double y2)
	{
	...
	public void doKMeans()
	{
		while (true)
		{
			...
			for (int i = 0; i < CooperativeObservation.NUM_TARGETS; i++)
			{
				...
				{
					double currDist = distanceBetweenPointsSQR(
							co.targetPos[i].x, co.targetPos[i].y,
			...
			for (int i = 0; i < CooperativeObservation.NUM_TARGETS; i++)
			{
				if (labels[i] != -1)
				{
					means[labels[i]] = new Double2D(means[labels[i]].x
							+ co.targetPos[i].x, means[labels[i]].y
							+ co.targetPos[i].y);
					n[labels[i]]++;
				}
			}
			for (int i = 0; i < CooperativeObservation.NUM_AGENTS; i++)
			{
				synchronized (usable[i])
				{
					if (n[i] != 0)
					{
						means[i] = new Double2D(means[i].x / n[i], means[i].y
								/ n[i]);
						clusterPoints[i] = new Double2D((1 - ALFA)
								* clusterPoints[i].x + ALFA * means[i].x,
								(1 - ALFA) * clusterPoints[i].y + ALFA
										* means[i].y);
